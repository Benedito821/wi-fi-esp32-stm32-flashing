...
/**
 *http server monitor task usedd to track events of the http server
...
/**
 * app.css get handdler is requested when accessing the web page
 * @param req HTTP request for which the uri needs to be 
 * @return ESP_OK
*/
...
/**
 * app.js get handdler is requested when accessing the web page
 * @param req HTTP request for which the uri needs to be handled
 * @return ESP_OK
*/
...
/**
 * Receives the .bin files via the web page and handles the firmware update
 * @param req http request for which the uri needs to be handled
...
esp_err_t http_server_OTA_update_handler(httpd_req_t *req)
{
    ...
    do
    {
        ...
        //is this the first data er are receiving
...
/**
 * MPU sensor readings JSON handler responds with MPU6050 sensor data
 * @param req HTTP request for which the uri needs to be handled
 * @return ESP_OK
 */
static esp_err_t http_server_get_mpu_sensor_readings_json_handler(httpd_req_t *req)
{
	ESP_LOGI(TAG, "/mpuSensor.json requested");

	char mpuSensorJSON[100];
    ts_mpu6050* temp_meas = mpu6050_export_obj();
	sprintf(mpuSensorJSON, "{\"temp\":\"%.2f\"}", temp_meas->get_temperature(I2C_NUM_0));
    // ESP_LOGI(TAG, "temperature = %.2fºC ",temp_meas);
	httpd_resp_set_type(req, "application/json");
	httpd_resp_send(req, mpuSensorJSON, strlen(mpuSensorJSON));
...
static httpd_handle_t http_server_configure(void)
{
    ...
    config.stack_size = HTTP_SERVER_MONITOR_STACK_SIZE;
    ...
    if(httpd_start(&http_server_handle,&config )== ESP_OK)
    {
        ...
        //register mpuSensor.json handler
         httpd_uri_t mpuSensor_json = {
                .uri = "/mpuSensor.json",
                .method = HTTP_GET,
                .handler = http_server_get_mpu_sensor_readings_json_handler,
                .user_ctx = NULL
         };
         httpd_register_uri_handler(http_server_handle,&mpuSensor_json);
        ...
        // register local_time.json handler
